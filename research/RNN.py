...
class PyTorchRNN(nn.Module):
    def __init__(self, in_features, hidden_dim, num_layers, dropout):
        ...
        self.rnn = nn.RNN(in_features, hidden_dim, num_layers=num_layers, batch_first=True, dropout=dropout if num_layers > 1 else 0.0)
...
for exp_idx, (exp_name, feat_cols) in enumerate(experiments):
    ...
    def objective_exp(trial):
        ...
        for fold_idx, (tr_idx, val_idx) in enumerate(skf.split(idx_tr, y_all[idx_tr])):
            ...
            model = PyTorchRNN(X_feat_matrix.shape[1], hidden_dim, num_layers, dropout).to(DEVICE)
            ...
            cv_scores.append(average_precision_score(Y_va_fold, val_probs))
    ...
    for fold_idx, (tr_idx, val_idx) in enumerate(skf.split(idx_tr, y_all[idx_tr])):
        ...
        train_loader = DataLoader(train_ds, batch_size=4096, shuffle=True, num_workers=0)
        fold_model = PyTorchRNN(X_feat_matrix.shape[1], best_p['hidden_dim'], best_p['num_layers'], best_p['dropout']).to(DEVICE)
        ...
        model_pkl_path = os.path.join(OUT_MODEL_DIR, f"RNN_{exp_name}_fold{fold_idx}.pkl")
    ...
    json_save_path = os.path.join(OUT_JSON_DIR, "result_RNN.json")
...
print("\n=======================================================")
print("       RNN FEATURE ABLATION SUMMARY REPORT             ")
print("=======================================================")
