...
# =====================================================
# STARTUP MEMORY & PYCACHE CLEANUP
# =====================================================
gc.collect()
...
for _mod_name in ["torch.utils.data.dataset", "torch.utils.data.dataloader", "torch.utils.data._utils.fetch"]:
    try:
        _mod = importlib.import_module(_mod_name)
        _cache = pathlib.Path(_mod.__file__).parent / "__pycache__"
        if _cache.exists():
...
class PyTorchLSTM(nn.Module):
    def __init__(self, in_features, hidden_dim, num_layers, dropout):
        super(PyTorchLSTM, self).__init__()
        self.lstm = nn.LSTM(in_features, hidden_dim, num_layers=num_layers, batch_first=True, dropout=dropout if num_layers > 1 else 0.0)
...
for exp_idx, (exp_name, feat_cols) in enumerate(experiments):
    ...
    def objective_exp(trial):
        ...
        for fold_idx, (tr_idx, val_idx) in enumerate(skf.split(idx_tr, y_all[idx_tr])):
            fold_tr_idx, fold_va_idx = idx_tr[tr_idx], idx_tr[val_idx]
            ...
            train_ds = TensorDataset(torch.tensor(X_tr_sub, dtype=torch.float32), torch.tensor(Y_tr_sub, dtype=torch.float32))
            train_loader = DataLoader(train_ds, batch_size=4096, shuffle=True, num_workers=0)
            model = PyTorchLSTM(X_feat_matrix.shape[1], hidden_dim, num_layers, dropout).to(DEVICE)
            ...
            del model, criterion, optimizer, train_loader, train_ds, X_tr_fold, Y_tr_fold, X_va_fold, Y_va_fold, X_tr_sub, Y_tr_sub
    ...
    print(f"\n=======================================================")
    print(f"  [STEP 2: EVALUATION] {N_SPLITS}-Fold CV Evaluation for '{exp_name}'")
    print(f"=======================================================")
    ...
    for fold_idx, (tr_idx, val_idx) in enumerate(skf.split(idx_tr, y_all[idx_tr])):
        ...
        train_ds = TensorDataset(torch.tensor(X_tr_f, dtype=torch.float32), torch.tensor(Y_tr_f, dtype=torch.float32))
        train_loader = DataLoader(train_ds, batch_size=4096, shuffle=True, num_workers=0)

        fold_model = PyTorchLSTM(X_feat_matrix.shape[1], best_p['hidden_dim'], best_p['num_layers'], best_p['dropout']).to(DEVICE)
        ...
        model_pkl_path = os.path.join(OUT_MODEL_DIR, f"LSTM_{exp_name}_fold{fold_idx}.pkl")
        ...
        del fold_model, criterion, optimizer, train_loader, train_ds, X_tr_full_f, Y_tr_full_f, X_tr_f, Y_tr_f, X_va_f, Y_va_f
        if torch.cuda.is_available(): torch.cuda.empty_cache()
        gc.collect()
    ...
    json_save_path = os.path.join(OUT_JSON_DIR, "result_LSTM.json")
    with open(json_save_path, "w", encoding="utf-8") as f:
        json.dump(ablation_results, f, indent=2, ensure_ascii=False)

print("\n=======================================================")
print("        LSTM FEATURE ABLATION SUMMARY REPORT           ")
print("=======================================================")
