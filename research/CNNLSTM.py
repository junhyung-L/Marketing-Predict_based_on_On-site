...
class PyTorchCNNLSTM(nn.Module):
    def __init__(self, in_features, filters, lstm_units, dropout):
        super(PyTorchCNNLSTM, self).__init__()
        self.conv1 = nn.Conv1d(1, filters, kernel_size=3, padding=1)
...
for exp_idx, (exp_name, feat_cols) in enumerate(experiments):
    ...
    def objective_exp(trial):
        ...
        for fold_idx, (tr_idx, val_idx) in enumerate(skf.split(idx_tr, y_all[idx_tr])):
            ...
            model = PyTorchCNNLSTM(X_feat_matrix.shape[1], filters, lstm_units, dropout).to(DEVICE)
    ...
    for fold_idx, (tr_idx, val_idx) in enumerate(skf.split(idx_tr, y_all[idx_tr])):
        ...
        train_loader = DataLoader(train_ds, batch_size=4096, shuffle=True, num_workers=0)
        fold_model = PyTorchCNNLSTM(X_feat_matrix.shape[1], best_p['filters'], best_p['lstm_units'], best_p['dropout']).to(DEVICE)
        ...
        with open(os.path.join(OUT_MODEL_DIR, f"CNNLSTM_{exp_name}_fold{fold_idx}.pkl"), "wb") as f:
    ...
    with open(os.path.join(OUT_JSON_DIR, "result_CNNLSTM.json"), "w", encoding="utf-8") as f:
        json.dump(ablation_results, f, indent=2, ensure_ascii=False)
    del X_feat_matrix, raw_feat_matrix
    gc.collect()

print("\n=======================================================")
print("       CNN-LSTM FEATURE ABLATION SUMMARY REPORT        ")
print("=======================================================")
print(json.dumps(ablation_results, indent=2))
print(f"\n[SAVED] JSON Report successfully saved.")
