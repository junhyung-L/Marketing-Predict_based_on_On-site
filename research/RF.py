...
for exp_idx, (exp_name, feat_cols) in enumerate(experiments):
    ...
    X_feat_matrix = df[feat_cols].fillna(0).to_numpy(dtype=np.float32)
    ...
    def objective_exp(trial):
        ...
        for fold_idx, (tr_idx, val_idx) in enumerate(skf.split(idx_tr, y_all[idx_tr])):
            ...
            val_probs = rf.predict_proba(X_va_fold)[:, 1]
            pr_auc = average_precision_score(Y_va_fold, val_probs)
            cv_scores.append(pr_auc)

            del rf, X_tr_fold, Y_tr_fold, X_va_fold, Y_va_fold, X_tr_sub, Y_tr_sub
            gc.collect()
    ...
    for fold_idx, (tr_idx, val_idx) in enumerate(skf.split(idx_tr, y_all[idx_tr])):
        ...
        fold_model = RandomForestClassifier(
            ...
            class_weight={0: 1.0, 1: best_p['pos_weight']},
            n_jobs=1,
            random_state=SEED + fold_idx
        )
        fold_model.fit(X_tr_f, Y_tr_f)

        model_pkl_path = os.path.join(OUT_MODEL_DIR, f"RF_{exp_name}_fold{fold_idx}.pkl")
    ...
    json_save_path = os.path.join(OUT_JSON_DIR, "result_RF.json")
    with open(json_save_path, "w", encoding="utf-8") as f:
        json.dump(ablation_results, f, indent=2, ensure_ascii=False)

print("\n=======================================================")
print("     Random Forest FEATURE ABLATION SUMMARY REPORT      ")
print("=======================================================")
