...
experiments = [
    # ("base",  feat_cols_base),  # already completed
...
for exp_idx, (exp_name, feat_cols) in enumerate(experiments):
    ...
    def objective_exp(trial):
        ...
        for fold_idx, (tr_idx, val_idx) in enumerate(skf.split(idx_tr, y_all[idx_tr])):
            ...
            model = MLPModel(X_feat_matrix.shape[1], hidden_dim, dropout).to(DEVICE)
    ...
    for fold_idx, (tr_idx, val_idx) in enumerate(skf.split(idx_tr, y_all[idx_tr])):
        ...
        fold_model = MLPModel(X_feat_matrix.shape[1], best_p['hidden_dim'], best_p['dropout']).to(DEVICE)
        ...
        model_pkl_path = os.path.join(OUT_MODEL_DIR, f"MLP_{exp_name}_fold{fold_idx}.pkl")
    ...
    json_save_path = os.path.join(OUT_JSON_DIR, "result_MLP.json")
    with open(json_save_path, "w", encoding="utf-8") as f:
        json.dump(ablation_results, f, indent=2, ensure_ascii=False)

print("\n=======================================================")
print("        MLP FEATURE ABLATION SUMMARY REPORT            ")
print("=======================================================")
